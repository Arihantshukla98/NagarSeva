...
class InfiniteGridMenu {
  ...
  #init(onInit: ((instance: InfiniteGridMenu) => void) | null) {
    ...
    const discProg = createProgram(gl, [discVertShaderSource, discFragShaderSource], undefined, {
      aModelPosition: 0,
      aModelNormal: 1,
      aModelUvs: 2,
      aInstanceMatrix: 3
  ...
  #initTexture() {
    ...
    // Prefill background to prevent any black lines
    ...
    Promise.all(
      this.items.map(
        (item, index) =>
          new Promise<HTMLImageElement | HTMLCanvasElement>(resolve => {
            ...
            img.onerror = () => {
              console.warn(`Failed to load WebGL card image: ${item.title || 'Civic Issue'}, compiling custom procedural canvas fallback.`);
              
              const pCanvas = document.createElement('canvas');
              pCanvas.width = cellSize;
              pCanvas.height = cellSize;
              const pCtx = pCanvas.getContext('2d')!;

              // Custom colorful gradient background corresponding to category colors
              const grad = pCtx.createLinearGradient(0, 0, cellSize, cellSize);
              const colorSchemes = [
              ...
              grad.addColorStop(0, scheme[0]);
              grad.addColorStop(1, scheme[1]);

              pCtx.fillStyle = grad;
              pCtx.fillRect(0, 0, cellSize, cellSize);

              // Circular subtle lens overlay
              pCtx.fillStyle = 'rgba(255, 255, 255, 0.12)';
              pCtx.beginPath();
              pCtx.arc(cellSize * 0.5, cellSize * 0.5, cellSize * 0.35, 0, Math.PI * 2);
              pCtx.fill();

              // Overlay nice typography text/icon
              ...
              const textToShow = item.title ? item.title.substring(0, 16) + (item.title.length > 16 ? '...' : '') : 'NagarSeva';
              pCtx.fillText(textToShow, cellSize / 2, cellSize / 2);

              // Subtext description
              pCtx.font = '500 20px monospace';
              pCtx.fillStyle = 'rgba(255, 255, 255, 0.85)';
              pCtx.fillText('CIVIC OUTPOST', cellSize / 2, cellSize / 2 + 50);
      ...
      console.error('Critical texture atlas generation failed:', err);
    });
  }
  ...
  #render() {
    ...
    // Alpha support for transparent WebGL background
  ...
  #onControlUpdate(deltaTime: number) {
    const timeScale = deltaTime / this.TARGET_FRAME_DURATION + 0.001;
...
const defaultItems = [
  {
    image: 'https://images.unsplash.com/photo-1594818856205-5022f49479a3?q=80&w=800',
    link: '#',
    title: 'Civic Report',
    description: 'Active ticket listing.'
  }
];
